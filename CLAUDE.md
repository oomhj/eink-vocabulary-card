...
- **运行/电源** — `ESP.deepSleep(60s, WAKE_RF_DISABLED)` 每 60s 自动刷词；RST(SW4) 手动刷（一次=一个词）。唤醒=整机重启，工作态存 RTC 内存（`State`+magic，跨深睡），断电保持存 LittleFS（`/wc_state.bin`，每 10 次刷词存一次省磨损）。**睡眠恢复（2026-08 硬件修复）**：此前深睡定时唤醒挂死 / RST 需按两次，根因是硬件——IO7（SPIQ，flash 数据线）悬空使 boot ROM 读 flash 偶发失败（`ets` 后无 `load`）；**IO7 焊盘 10k 上拉 3.3V 后深睡定时唤醒与 RST 单按均正常**（见 `docs/hardware.md`）。FPM light sleep 为固件死路（RF 关=不睡、RF 开=`wifi_fpm_do_sleep` 空指针崩溃，Arduino core 3.x 移除 `ESP.lightSleep()` 的原因），勿再尝试。
