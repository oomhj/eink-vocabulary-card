...
- 每 60 秒自动刷一张**随机词卡**（单词 + 音标 + 中文释义 + 电量 + 轮次页码）——正常运行不休眠（睡眠方案实测不可用，见 `src/word_cards.cpp` 注记）
- SW2（上键 IO0）按键刷一张词，一次按下 = 一个词；RST（SW4）整机重启刷一张词
- 快刷为主、每 10 次全刷清残影；计数/轮次经 LittleFS 断电保持
...
- 引脚：11 个 GPIO 全部专用（屏 SPI×6 + 屏控制×2 + 电池采样 + 上键 SW2 + GPIO16（深睡唤醒脚，固件当前未用）+ UART0），无富余脚
